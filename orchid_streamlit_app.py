...
class OrchidSearchDB:
    ...
    def load_data_from_url(self, url: str):
        ...
        # Populate FTS table
        self.cursor.execute("INSERT INTO orchids_fts(orchids_fts) VALUES('rebuild')")
        self.conn.commit()
...
if st.session_state.data_loaded:
    # Search tabs
    tab1, tab2, tab3 = st.tabs(["🔍 Full-Text Search", "🎯 Advanced Filter", "📊 Browse All"])
                ...
                try:
                    ...
                    if results:
                        ...
                        for i, result in enumerate(results, 1):
                            with st.expander(f"**{i}. {result['Scientific_Name']}** - {result['Common_Names']}"):
                                col1, col2, col3 = st.columns(3)
                                
                                with col1:
                                    st.markdown("**🌺 Flower Details**")
                                    st.write(f"**Color:** {result['Flower_Color']}")
                                    st.write(f"**Size:** {result['Flower_Size_cm']} cm")
                                    st.write(f"**Shape:** {result['Petal_Shape']}")
                                    st.write(f"**Fragrance:** {result['Fragrance']}")
                                
                                with col2:
                                    st.markdown("**🌡️ Growing Conditions**")
                                    st.write(f"**Temperature:** {result['Temperature_Min_C']}-{result['Temperature_Max_C']}°C")
                                    st.write(f"**Humidity:** {result['Humidity_Min_Percent']}-{result['Humidity_Max_Percent']}%")
                                    st.write(f"**Light:** {result['Light_Description']}")
                                    st.write(f"**Difficulty:** {result['Horticultural_Difficulty']}")
                                
                                with col3:
                                    st.markdown("**🌍 Origin & Habitat**")
                                    st.write(f"**Region:** {result['Native_Regions']}")
                                    st.write(f"**Habitat:** {result['Native_Habitat']}")
                                    st.write(f"**Climate:** {result['Climate_Type']}")
                                    st.write(f"**Blooming:** {result['Blooming_Season']}")
            ...
            with st.spinner("Filtering..."):
                ...
                if results:
                    ...
                    for i, result in enumerate(results[:10], 1):
                        with st.expander(f"{i}. {result['Scientific_Name']}"):
                            col1, col2 = st.columns(2)
                            
                            with col1:
                                st.markdown("**Basic Information**")
                                st.write(f"**Common Name:** {result['Common_Names']}")
                                st.write(f"**Genus:** {result['Genus']}")
                                st.write(f"**Family:** {result['Family']}")
                                st.write(f"**Growth Habit:** {result['Growth_Habit']}")
                            
                            with col2:
                                st.markdown("**Care Requirements**")
                                st.write(f"**Difficulty:** {result['Horticultural_Difficulty']}")
                                st.write(f"**Watering:** {result['Watering_Frequency']}")
                                st.write(f"**Potting Media:** {result['Potting_Media']}")
                                st.write(f"**Fertilizer:** {result['Fertilizer_Requirement']}")
                else:
